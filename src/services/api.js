...
export const design = {
  ...
  experts: (includeInactive = false) => request(`/design/experts${includeInactive ? '?all=1' : ''}`),
  createExpert: (name) => request('/design/experts', { method: 'POST', body: JSON.stringify({ name }) }),
  toggleExpert: (id) => request(`/design/experts/${id}/toggle-active`, { method: 'PATCH' }),
  deleteExpert: (id) => request(`/design/experts/${id}`, { method: 'DELETE' }),
